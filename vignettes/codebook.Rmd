...
var_sections <- function(data,
  ...
  data %>% purrr::pmap(function(variable_name,
    ...
    if (is_opt) {
      c("#### Origin",
        "",
        glue::glue("This variable is derived from ", pal::prose_ls(rdb:::md_link_codebook(unlist(derived_from))), " and can be generated on demand via ",
                   ifelse(strip_extended_pandoc,
                          "[{gen_fn}()]",
...
[^multi-val-var]: A multi-value variable allows for more than one value per observation and thus breaks with the [tidy-data convention](https://tidyr.tidyverse.org/articles/tidy-data.html). Use [`tidyr::unnest_longer()`](https://tidyr.tidyverse.org/reference/hoist.html) to convert data containing such a variable to long format. To unnest `topics_tier_1` to `topic_tier_1` for example, use:
    ...
    \`\`\`r
    rdb::rfrnds() %>% tidyr::unnest_longer(col = topics_tier_1, values_to = \"topic_tier_1\")
    \`\`\`
")
```
