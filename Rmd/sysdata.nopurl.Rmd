...
-   Due to lack of official revocation information in ISO 3166-2, we construct our own (delayed) `valid_to` date column based on detected changes in
    `ISOcodes::ISO_3166_2` and just set `valid_from` to the dummy value `0101-01-01`. Ideally, the actual dates would be added to the
...
data_iso_3166_2 <-
  ...
  dplyr::rename_with(.cols = everything(),
                     .fn = heck::to_snake_case) |>
  dplyr::mutate(valid_from = clock::date_parse("0101-01-01"),
                valid_to = as.Date(NA)) |>
  dplyr::relocate(parent_code = parent,
                  .after = code) |>
  # add expired codes
...
-   Add more countries which have an [official municipality identifier](https://en.wikipedia.org/wiki/Community_Identification_Number).
    ...
    -   `AT`: [ÖSTAT-Nr](https://de.wikipedia.org/wiki/Amtlicher_Gemeindeschl%C3%BCssel#%C3%96sterreich)
    -   `DE`: [Amtlicher Gemeindeschlüssel (AGS)](https://de.wikipedia.org/wiki/Amtlicher_Gemeindeschl%C3%BCssel#Deutschland)
...
-   Unfortunately, there is no ISO code standard for municipalities or any other standard with full coverage of all municipalities worldwide. We resort to the
    *UN/LOCODE*, the [United Nations Code for Trade and Transport Locations](https://en.wikipedia.org/wiki/UN/LOCODE), as the best available global municipality
    identifier. UN/LOCODE covers more than just municipalities, including entities like seaports, rail and road terminals, airports or border crossing points –
...
derive_subnational_entity_code <- function(id,
  ...
  parent <-
    data_municipalities |>
    dplyr::filter(Identifier == !!id & Level %in% !!levels) |>
  ...
  if (is.na(parent$Parent)) {
    ...
    result <- derive_subnational_entity_code(id = parent$Parent,
                                             levels = 1:2)
  }
...
data_municipalities <- swissmuni::snapshots(start_date = "0101-01-01",
...
data_municipalities %<>%
  ...
  dplyr::mutate(country_code = "CH",
                subnational_entity_code = purrr::map_chr(Parent,
                                                         \(id) derive_subnational_entity_code(id = id,
                                                                                              levels = 1:2))) %>%
  # tidy cols
  dplyr::select(country_code,
                ...
                valid_to = ValidTo) %>%
  # remove spurious rows with `valid_from == valid_to` which is i.a. necessary to fulfill `UNIQUE (country_code, id_official, valid_from)` constraint
  # REMARK: the duplicates for [Bischofszell](https://de.wikipedia.org/wiki/Bischofszell) e.g. are likely caused by because it was a
  #         [Einheitsgemeinde](https://de.wikipedia.org/wiki/Einheitsgemeinde#Kanton_Thurgau) up until 1987
  dplyr::filter(valid_from != valid_to | is.na(valid_to)) %>%
  # because municipalities that only existed in the period before the first allocation of FSO numbers in 1960 were given the FSO number 7700,
  # replace `id_official == 7700` with custom unique one
  dplyr::mutate(id_official = dplyr::if_else(id_official == "7700",
                                             paste(id_official, name,
                                                   sep = "-"),
                                             id_official)) %>%
  # add our municipality ID
  dplyr::mutate(id = paste(country_code, id_official, valid_from,
                           sep = "_"),
                .before = 1L)

# ensure `UNIQUE (country_code, id_official, valid_from)` constraint
invalid_data <-
  data_municipalities |>
  dplyr::group_by(country_code, id_official, valid_from) |>
  dplyr::summarise(n = dplyr::n(),
                   .groups = "drop_last") |>
  dplyr::filter(n > 1L)

if (nrow(invalid_data) > 0L) {
  cli::cli_abort("{.code UNIQUE (country_code, id_official, valid_from)} constraint is still violated. Please debug.")
}
...
data_un_locode <-
  ...
  ## CH Schönenbuch: we keep `CH SOH` which was created first
  dplyr::filter(un_locode != "CH SBC") |>
  ## duplicated rows (since 2024-11-07 the source data contains duplicates)
  unique()
...
-   `data-raw/political_topics.json` was sourced from private CCM Design repo via:
    ...
    yay::gh_text_file(owner = "zdaarau",
      ...
      brio::write_file(path = "data-raw/political_topics.json")
    ```

```{r}
data_topics <-
  jsonlite::fromJSON(txt = "data-raw/political_topics.json",
                     flatten = FALSE) |>
  tibble::as_tibble() |>
...
pg_tbl_metadata <-
  ...
  magrittr::set_names(., .) |>
  purrr::map(\(x) rdb:::pg_col_metadata(tbl_name = x,
...
pkg_config <-
  ...
  tibble::add_row(key = "pg_roles_csv_file",
                  ...
                  require = TRUE) |>
  tibble::add_row(key = "neon_api_key",
                  description = paste0("[API key](https://neon.tech/docs/manage/api-keys) for the RDB neon.tech project ",
                                       pal::wrap_chr(rdb:::neon_project, "\x60"),
                                       ". Required for [rdb::restart_neon_ep()]."),
                  require = TRUE) |>
  tibble::add_row(key = "nocodb_origin",
                  default_value = list("https://admin.rdb.vote"),
                  description = "RDB NocoDB server [origin](https://developer.mozilla.org/docs/Glossary/Origin). Required for [rdb::reset_nocodb()].",
                  require = TRUE) |>
  tibble::add_row(key = "nocodb_email",
                  description = "E-mail address of the RDB NocoDB server's super admin user. Required for [rdb::reset_nocodb()].",
                  require = TRUE) |>
  tibble::add_row(key = "nocodb_password",
                  description = "Password of the RDB NocoDB server's super admin user. Required for [rdb::reset_nocodb()].",
                  require = TRUE) |>
  tibble::add_row(key = "nocodb_user_account_csv_file",
                  description = paste0("File path to a UTF-8 encoded CSV file containing the columns `name`, `email` and `password` describing additional user",
                                       " accounts to create on the RDB NocoDB server via [rdb::reset_nocodb()]."),
                  require = FALSE) |>
  tibble::add_row(key = "nocodb_s3_access_key",
                  description = paste0("ID of an access key granting read/write access to the S3-compatible object storage buckets `rdb-attachments` and ",
                                       "`rdb-nocodb`. Required for various [administration functions](https://rdb.rpkg.dev/reference/#administration)."),
                  require = TRUE) |>
  tibble::add_row(key = "nocodb_s3_access_secret",
                  description = paste0("Secret of an access key granting read/write access to the S3-compatible object storage buckets `rdb-attachments` and ",
                                       "`rdb-nocodb`. Required for various [administration functions](https://rdb.rpkg.dev/reference/#administration)."),
                  require = TRUE) |>
  tibble::add_row(key = "fly_io_api_token",
                  description = paste0("Fly.io [API token](https://fly.io/docs/flyctl/tokens/) granting access to the RDB organization. Required for ",
                                       "[rdb::restart_postgrest()]."),
                  require = TRUE) |>
...
if (pal::desc_dep_vrsn(pkg = "ISOcodes") < max(as.numeric_version(pal::ls_pkg(pkg = "ISOcodes")$Version))) {
  ...
  cli::cli_alert_warning(paste0("New ISOcodes package version in use. Please run {.run pal::use_pkg(\"ISOcodes\", type = \"Suggests\")'}, then rebuild the ",
                                "package, and finally run {.run {paste0('rdb::', c('update_administrative_units()', 'update_langs()'))}} to update the ",
                                "corresponding RDB PostgreSQL tables."))
}
```
...
-   We just fetch the root certificate from [Let's Encrypt](https://letsencrypt.org/certificates/).

```{r}
download.file(url = "https://letsencrypt.org/certs/isrgrootx1.pem",
              destfile = "inst/certs/ISRG_Root_X1.pem",
              quiet = TRUE,
              mode = "wb")
```

# Regenerate RDB data model diagram

```{r}
fs::dir_create(path = "man/figures",
               recurse = TRUE)
rdb::dm() |>
  dm::dm_draw(view_type = "all") |>
  DiagrammeRsvg::export_svg() |>
  brio::write_file(path = "man/figures/rdb_dm.svg")

# also write a PNG version for inclusion in image links
rsvg::rsvg_png(svg = "man/figures/rdb_dm.svg",
               file = "man/figures/rdb_dm.png",
               width = 2500L)
```
