...
pkg_config <-
  ...
  tibble::add_row(key = "nocodb_hostname",
                  description = paste0("Hostname of the [RDB NocoDB server](https://admin.rdb.vote/). Used as default for the `*nococdb*()` family of ",
                                       "functions.")) |>
  tibble::add_row(key = "nocodb_api_token",
                  description = paste0("NocoDB [API authentication token](https://docs.nocodb.com/account-settings/api-tokens/) for the [RDB NocoDB ",
                                       "server](https://admin.rdb.vote/). Used as default for the `*nococdb*()` family of functions.")) |>
  tibble::add_row(key = "nocodb_base_title",
                  default_value = list("Main"),
                  description = paste0("Title (name) of the main base on the [RDB NocoDB server](https://admin.rdb.vote/). Used as default for the ",
                                       "`*nococdb*()` family of functions.")) |>
  ...
  tibble::add_row(key = "pg_user",
                  ...
                  description = paste0("RDB PostgreSQL username. To be able to edit the database, the user must have sufficient privileges. Defaults to ",
                                       "the read-only user `r_anon`.")) |>
  tibble::add_row(key = "pg_password",
                  default_value = list("WTYUeDtTm0UV4nXzTZ7morHSdM0wQh0p"),
                  description = "Password for the specified RDB PostgreSQL user. Defaults to the password for user `r_anon`.") |>
