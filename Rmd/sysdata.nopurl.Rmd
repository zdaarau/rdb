...
-   Due to lack of official revocation information in ISO 3166-2, we construct our own (delayed) `valid_to` date column based on detected changes in
    `ISOcodes::ISO_3166_2` and just set `valid_from` to the dummy value `0001-01-01`. Ideally, the actual dates would be added to the
...
data_iso_3166_2 <-
  ...
  dplyr::rename_with(.cols = everything(),
                     .fn = heck::to_snake_case) |>
  dplyr::mutate(valid_from = clock::date_parse("0001-01-01"),
                valid_to = as.Date(NA)) |>
...
data_municipalities <- swissmuni::snapshots(start_date = "0001-01-01",
...
pkg_config <-
  ...
  tibble::add_row(key = "neon_api_key",
                  description = paste0("[API key](https://neon.tech/docs/manage/api-keys) for the RDB neon.tech project `r pal::wrap_chr(rdb:::neon_project, ",
                                       "'\x60')`. Required for [rdb::restart_neon_instance()]."),
                  require = TRUE) |>
  ...
  tibble::add_row(key = "nocodb_s3_access_key",
                  description = paste0("ID of an access key granting read/write access to the S3-compatible object store holding the `rdb-attachments` bucket.",
                                       " Required for various [administration functions](https://rdb.rpkg.dev/reference/#administration)."),
                  require = TRUE) |>
  tibble::add_row(key = "nocodb_s3_access_secret",
                  description = paste0("Secret of an access key granting read/write access to the S3-compatible object store holding the `rdb-attachments` ",
                                       "bucket. Required for various [administration functions](https://rdb.rpkg.dev/reference/#administration)."),
                  require = TRUE) |>
