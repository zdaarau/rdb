...
codes <-
  ...
  dplyr::mutate(un_country_code = dplyr::if_else(country_code == "TW",
                                                 "156",
                                                 un_country_code))
...
```{r}
val_set <- list()
val_set$country_code <- c(ISOcodes::ISO_3166_1$Alpha_2,
...
```{r}
rfrnd_cols_order <-
  data_codebook$variable_name %>%
  ## country_code_long after country_code
  c2d:::insert_after(new = "country_code_long",
                     after = "country_code") %>%
  ## period cols after `date`
  c2d:::insert_after(new = c("week", "month", "quarter", "year", "decade", "century"),
                     after = "date") %>%
  ## cols `un_tier_*` after `municipality`
  c2d:::insert_after(new = setdiff(colnames(un_regions),
                                   "country_code"),
                     after = "municipality") %>%
  ## `turnout` after `votes_per_subterritory`
  c2d:::insert_after(new = "turnout",
                     after = "votes_per_subterritory")
```
