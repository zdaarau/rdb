...
-   Depending on the referendum(s), the number of fields returned by the C2D API varies. To systematically test if `tidy_referendums()` works for all cases,
    run:

    ``` r
    ids = c2d::referendums(use_cache = FALSE) %$% id

    ids %>%
      cli::cli_progress_along(name = "Test") %>%
      purrr::walk(~ {
        cli::cli_text("i {.val {.x}} with id {.field {ids[.x]}}")
        c2d::referendum(id = ids[.x])
      })
    ```
