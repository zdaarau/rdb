...
TODO: Remove this as soon as [issue \#53](https://github.com/ccmdesign/c2d-app/issues/53) is resolved.
...
# [sudd.ch](https://sudd.ch/) scraping

## `sudd_referendums`

TODO:

-   Parse `country_name_de` into `country_name_de`, `subnational_entity_name_de`, `municipality_name_de`; see <https://sudd.ch/list.php?mode=allareas> for how
    territories are clustered.

-   Test with all `c2d::referendums()$id_sudd`
...
sudd_referendums <- function(ids_sudd) {
  ...
  ids_sudd %>%
    ...
    purrr::map_dfr(sudd_referendum)
}

# get a single referendum's data from sudd.ch
sudd_referendum <- function(id_sudd) {
  ...
  xml2::read_html(paste0("https://sudd.ch/event.php?id=", id_sudd)) %>%
    ...
    # extract `country_code` from `id_sudd`
