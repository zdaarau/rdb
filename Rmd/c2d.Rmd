...
Functions to work with referendum [topics](https://rpkg.dev/c2d/articles/codebook.html#topics).
...
Functions to visualize the C2D referendum data (using [plotly](https://plotly-r.com/)).
...
-   Unless either
    ...
    b)  topics are stored in a non-deduplicated way per tier (e.g. in a tibble as returned by `c2d::hierarchize_topics()`),
...
-   Function is computationally heavy (i.e. slow) because of the above. The `c2d::hierarchize_topics_fast()` call per row still takes too much time. Maybe there
...
-   `filter` seems buggy -\> investigate! e.g. `c2d::list_sudd_rfrnds(mode = "by_mod_date", filter = list(territory_name_de = "ascension"))`
