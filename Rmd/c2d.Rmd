...
sudd_referendum <- function(id_sudd) {
  ...
  field_names %<>%
    ...
    purrr::map_if(.p = ~ .x %>% stringr::str_detect(pattern = "^\u2517\u2501 "),
                  .f = function(old_name, old_names) {
                    paste0("votes_option_", which(old_names == old_name))
                  },
...
TODO: Properly parse remaining exotic cols:

``` {.r}
data_sudd <- readr::read_rds(file = "data_sudd.rds")
affected_cols <- data_sudd %>% colnames() %>% stringr::str_subset("[A-Z]") %>% sort()
data_sudd %>% dplyr::filter(dplyr::if_any(.cols = all_of(affected_cols), .fns = ~ !is.na(.x))) %$% id_sudd
```
...
sudd_referendums <- function(ids_sudd) {
  ...
  ids_sudd %<>% magrittr::extract(!is.na(.))
