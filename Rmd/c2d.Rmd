...
rename_from_list <- function(x,
  ...
  names_list_new <- names(x) %>% dplyr::recode(!!!names_list)
  
  x %>% magrittr::set_names(names_list_new)
}
```
...
tidy_referendums <- function(data,
  ...
  if (tidy) {
    ...
    data %<>%
      ...
      dplyr::mutate(
        # replace empty lists with `NULL` in *nested* list columns
        dplyr::across(.cols = where(is.list),
                      .fns = purrr::map,
                      .f = ~ {
                        if (length(.x) || !is.list(.x)) .x else NULL
                      }),
