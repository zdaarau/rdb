...
-   Move `ncdb_*` functions to separate pkg?

NOTES:

-   NocoDB exposes multiple separate APIs, i.a. [meta](https://meta-apis-v2.nocodb.com/) and [data](https://data-apis-v2.nocodb.com/) APIs.
    ...
    The OpenAPI description for a *base-specific* meta API OTOH is hosted under the URL `https://admin.rdb.vote/api/v2/meta/bases/<base_id>/swagger.json`.

-   Currently there are the following open-source solutions to automatically parse an OpenAPI description to be used in R:
...
-   Create `set_ncdb_col_metadata()` (analagous to `set_ncdb_tbl_metadata()`) and call it in `config_ncdb()`,

    -   setting the proper `uidt` and `meta.richMode` fields from `ncdb_col_metadata` where necessary (e.g. `attachments`, `remarks`etc.)
    -   removing the `referendum_` prefix from `title`s for virtual columns of `uidt == "Links"` in table `referendums`
    -   apply the `title` renamings from `ncdb_col_renames` using `ncdb_col_renames`
...
#### `upload_ncdb_attachments`

NOTES:

-   The [NocoDB API endpoint documentation](https://meta-apis-v2.nocodb.com/#tag/Storage) is very sparse and seems odd. According to the doc and [this
    comment](https://github.com/nocodb/nocodb/issues/4935#issuecomment-1406015536), it should be possible to define the `path` under which to store the uploaded
    file *including* directory parts, but all directory parts are simply ignored when provided in `curl::form_file(name = ...)` (percent-encoding slashes
    doesn't help). And according to the doc of `?httr2::req_body_multipart`, we must either rely on `curl::form_file()` (which doesn't allow to specify
    ...
    Therefore, we refrain from specifying any subdirs and let NocoDB upload files at the default location (`/nc/uploads/` in the `rdb-attachments` Backblaze B2
    bucket).
...
-   Viable alternative emojis for `ncdb_meta.icon` include:
