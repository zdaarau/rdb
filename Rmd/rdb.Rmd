...
pg_col_metadata <- function(tbl_name,
  ...
  if (!DBI::dbExistsTable(conn = connection,
    ...
    cli::cli_abort("No {.field {schema}.{tbl_name}} table found. Do you need to {.help rdb::reset_rdb} first?")
  }
  ...
  purrr::map_chr(x,
                 \(x) {
                   ...
                   if (!is.na(x) && stringr::str_detect(x, "https?:")) {
                     ...
                     httr2::url_modify(url = "https://sudd.ch",
                                       path = x %|% "")
                   }
                 })
}
...
tbl_metadata <-
  tibble::tribble(
    ...
    "electorate",                   "#b3b9ff",      "display",           "\U0001f465",      FALSE,   FALSE,
  ...
  dplyr::left_join(
    ...
    y = tibble::tribble(
      ...
      "electorate",                   "electorate per administrative unit and date",
...
-   This maps the field names of the back-end's MongoDB to the current variable names as found in the [RDB
    codebook](https://rdb.rpkg.dev/articles/codebook.html).
...
-   Can be deleted once migration from MongoDB to PGSQL DB is completely finished.
...
restore_rdb <- function(dm = readRDS(file = "rdb_dm.rds"),
  ...
  tbl_deps_left <-
    ...
    dm_excl_gen_always_cols(connection = connection) |>
    dm::dm_get_all_fks() |>
...
-   Finish backup&restore, i.e. debug `keep_data = TRUE`
...
Functions to administrate the [RDB NocoDB server](https://admin.rdb.vote/).
...
```{r, purl = FALSE}
#' Configure NocoDB table columns
#'
#' @description
#' Configures the RDB table columns on the specified NocoDB server. Currently, this includes:
#' 
#' - Setting ...
#'
#' @inheritParams nocodb::set_tbl_metadata
...
#' @family admin
#' @keywords internal
config_nocodb_tbl_cols <- function(origin = pal::pkg_config_val("nocodb_origin"),
                                   quiet = FALSE) {
...
Functions to administrate the [RDB PostgREST server](https://api.rdb.vote/).
