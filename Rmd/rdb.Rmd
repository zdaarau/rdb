...
-   Currently, we use the *Backblaze B2* NocoDB plugin. Ideally, we'd just use a vendor-neutral S3-compatible implementation instead. Once
    [nocodb/nocodb#8886](https://github.com/nocodb/nocodb/pull/8886) is merged, we should be able to switch to the *S3* plugin.

    When done, all the `b2_*` args below should be renamed to `s3_*` and the `nocodb::update_plugin()` call adapted accordingly.
