...
#### `upload_attachment_to_ncdb`

[`/api/v2/storage/upload`](https://meta-apis-v2.nocodb.com/#tag/Storage) API endpoint

```{r, purl = FALSE}
upload_attachment_to_ncdb <- function(auth_token = pal::pkg_config_val(key = "ncdb_token",
                                                                       pkg = this_pkg),
                                      max_tries = 3L,
                                      verbosity = NULL) {
  ...
  httr2::request(base_url = url_ncdb("api/v2/storage/upload")) |>
    ...
    httr2::req_headers(`xc-token` = auth_token) |>
    httr2::req_body_multipart(file = curl::form_file(path = "/home/salim/Downloads/MIDI_sample.mid",
                                                     name = "example.midi",
                                                     type = "audio/midi"),
                              # try without `json`!
                              json = curl::form_data(value = jsonlite::toJSON(list()),
