...
-   support multi-col PKs (e.g. `referendum_types_legal_norms`)
...
tbl_metadata <- tibble::tribble(
  ...
  "referendum_votes",             TRUE,         "#6673ff",      "option_display",          "\U0001F9FE",
...
-   We could also use `update_tbl()` (which builds upon `pg_tbl_update()`) here. But since the `municipalities` table has a
    `UNIQUE (country_code, id_official, valid_from)` constraint, we can leverage that, which is more elegant.

-   It doesn't make much sense to return the return value of `dplyr::rows_upsert()` since evaluating it will throw an `Invalid connection` error if
    `disconnect = TRUE` (the default). Hence, we just return `data_municipalities`.
...
-   Sometimes, `reset_rdb()` fails with
    ...
    `reset_rdb()` always finishes successfully on the second attempt, so we just live with it for now...
...
### `clean_rfrnd_attachments`

NOTES:

-   Merging [nocodb/nocodb#9140](https://github.com/nocodb/nocodb/pull/9140) should resolve [nocodb/nocodb#7895](https://github.com/nocodb/nocodb/issues/7895),
    which renders this fn largely obsolete. It might still be useful to clean up obsolete attachments added outside of NocoDB (TODO: check whether thumbnail
    generation works for them and whether they're properly tracked in NocoDB's new internal `MetaTable.FILE_REFERENCES`).
...
-   Currently, we use the *Backblaze B2* NocoDB plugin. Ideally, we'd just use the vendor-neutral S3 plugin instead which [allows configuration via the
    `NC_S3_*` environment variables](https://docs.nocodb.com/getting-started/self-hosted/environment-variables/#storage). Once
    [nocodb/nocodb#9054](https://github.com/nocodb/nocodb/issues/9054) is fixed, we should be able to switch to the *S3* plugin.

    When done, all the `b2_*` args below should be renamed to `s3_*` and the `nocodb::update_plugin()` call removed.
