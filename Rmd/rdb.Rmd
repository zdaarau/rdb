...
### Undefined global objects used in pipes
...
The R code chunk below is *not* included in the [source package](https://r-pkgs.org/structure.html#sec-source-package) (`purl = FALSE`) and thus has to be
...
-   Pkgs DBI and RPostgres respectively do not yet natively [support PostgreSQL array types](https://github.com/r-dbi/dbi3/issues/22), thus we have to convert
    them manually using this function.
...
-   There's apparently no dedicated option to vary the margin between an axis and its tick labels, only
    [`layout.yaxis.title.standoff`](https://plotly.com/r/reference/layout/yaxis/#layout-yaxis-title-standoff) to vary the margin between the axis title and the
    axis tick labels. Thus, we work around this by adding a trailing space to our `layout.yaxis.ticksuffix` which together with the default
    [layout.yaxis.automargin](https://plotly.com/r/reference/layout/yaxis/#layout-yaxis-automargin) results in a margin of a few pixels.
...
NOTE that this function doesn't restore *redundant* information (parent-tier topics). Once [issue #41](https://github.com/zdaarau/c2d-app/issues/41) is
...
-   Various base R methods like the `$` and `[[` extractor functions aren't implemented for `tbl_lazy` objects, so we have to rely on `dplyr::pull()`.

-   `nrow()` on a `lazy_tbl` [returns `NA`](https://github.com/tidyverse/dplyr/issues/2871) since actually implementing it [is considered a too expensive
    operation](https://github.com/tidyverse/dplyr/issues/2871#issuecomment-309461433). Thus, we resort to `dplyr::tally()`.

-   We can't overwrite tables with existing foreing key relations using `DBI::dbWriteTable(overwrite = TRUE)`. Hence we must update them in-place via
    [`dplyr::rows_*(in_place = TRUE)` functions](https://dplyr.tidyverse.org/reference/rows.html).
...
-   Since the `DROP DATABASE` command contained in `1-reset_db.sql` cannot run inside a transaction block, we cannot use `exec_sql_file()` which builds upon
...
Source: <https://stackoverflow.com/a/55046184/7196903>
...
Source: <https://stackoverflow.com/a/23390399/7196903>
...
pg_update_seq <- function(tbl_name,
  ...
  connection |>
    DBI::dbWithTransaction(code = {
      ...
      connection |>
        DBI::dbGetQuery(statement = glue::glue_sql(.con = connection,
                                                   "SELECT setval({seq_id},",
                                                   "              COALESCE(MAX({`col_name`}) + 1, 1),",
                                                   "              false)",
...
Source: <https://dba.stackexchange.com/a/214877/280534>
...
#' @param connection `r pkgsnip::type("pg_connection")`
...
pg_tbl_update <- function(tbl,
  ...
  are_pks_writable <-
    col_metadata |>
    dplyr::filter(is_pk) |>
    dplyr::select("identity_generation", "generated") |>
    unlist() |>
...
-   Consider improving this fn (see NOTES below) and moving it to pkg pal.

NOTES:

-   Does not yet support *nested* [dollar-quoted string constants](https://www.postgresql.org/docs/current/sql-syntax-lexical.html#SQL-SYNTAX-DOLLAR-QUOTING).
    This would require splitting by dollar-*tags*, which could be done by calling `stringr::str_locate()` repeatedly to determine split positions and then
    `stringr::str_sub()` instead of the `stringr::str_split_1()` used below.
...
-   An illuminating comparison of the currently available R clients for S3-compatible object storage is provided in [this
    comment](https://github.com/cloudyr/aws.s3/issues/433#issuecomment-1805612553).
...
-   Table columns are identified by `tbl_name` and

    -   `col_name` if non-virtual
    -   `title` and `uidt` if virtual

-   An `NA` value in non-identifiying columns means to leave the respective attribute untouched, i.e. at its default.
...
Define PostgreSQL metadata centrally.
...
-   The row order corresponds to the desired order of the tables in the NocoDB UI, i.e. to re-order tables, simply move them up/down in `tbl_metadata` and run
    `rdb::config_nocodb_tbls()`.

-   `\ufe0f` is the (optional) [Unicode image variation selector](https://stackoverflow.com/a/38100803/7196903) that specifies the emoji should be presented as
    an image; unclear whether this actually has any effect (but it doesn't harm anyways).

-   Viable alternative emojis for `nocodb_meta.icon` include:

    -   `legal_norms`: § (but no actual emoji and thus not recognized by NocoDB 🙁)
    -   `countries`: "\U0001F1FA\U0001F1F3"
    -   `municipalities`: "\U0001F3D8"
    -   `languages`: "\U0001F310"

-   Column meanings:

    -   `is_aux` indicates whether a DB table is auxiliary, i.e. is not part of the relational model of the RDB.
    -   `is_admin` indicates whether a DB table is administered fully programmatically and not intended to be altered by NocoDB users.
...
-   Once [cynkra/dm#1879](https://github.com/cynkra/dm/issues/1879) is fixed, we can use default R PG user to connect instead of tbl owner.
...
-   Join with additional db tbls; experiment with [`rdb::dm() |> dm::dm_flatten_to_tbl()`](https://dm.cynkra.com/reference/dm_flatten_to_tbl.html) and
    [`rdb::dm() |> dm::dm_wrap_tbl()`](https://dm.cynkra.com/reference/dm_wrap_tbl.html)
...
-   Either update this fn for PGSQL data schema or just remove it.
...
#### `clean_attachment_bucket`

NOTES:

-   [nocodb/nocodb#9215](https://github.com/nocodb/nocodb/pull/9215) laid groundwork to resolve
    [nocodb/nocodb#7895](https://github.com/nocodb/nocodb/issues/7895) (predecessor PR [nocodb/nocodb#9140](https://github.com/nocodb/nocodb/pull/9140) was
    closed unmerged), which in turn will render this fn largely obsolete when resolved.
...
TODO: Finish doc, including a succinct summary of all the info from `sysdata.nopurl.Rmd`.
...
The classification of political topics assigned to referendums was developed together with [Swissvotes](https://swissvotes.ch/), the [Institute of Federalism of
the University of Fribourg](https://www.unifr.ch/federalism/en/) and the [Section Politics of the Federal Statistical
...
-   Performance-wise, this fn is *not* optimized at all (it's really slow). Try to re-implement using `nest_topics()`!
...
-   Add better example which actually shows the difference between `country_code` and `country_code_continual`.
...
Functions to visualize the RDB referendum data using [plotly](https://plotly-r.com/) and [ggplot2](https://ggplot2.tidyverse.org/).
...
-   Unless either
    ...
    If we decide for a), we must adapt the spec in [issue #41](https://github.com/zdaarau/c2d-app/issues/41) accordingly and in the admin portal should nudge
...
-   Experiment replacing Plotly's default colors with `plotly::plot_ly(colors = viridis::turbo(n = ?))`
...
-   Experiment with `plotly::ggplotly()` to create an interactive streamgraph.
...
Functions to create analyses in tabular form as [gt](https://gt.rstudio.com/) tables.
...
-   Since we want to default to [`sslmode = verify-full`](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNECT-SSLMODE) but PostgreSQL
    doesn't know where to look for system-wide TLS certificates, we just ship the proper root certificate that Let's Encrypt certificates (which Neon.tech
    relies on) are signed with, [ISRG Root X1](https://letsencrypt.org/certificates/), with this package and explicitly set
    [`sslrootcert`](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNECT-SSLROOTCERT) accordingly.
...
### `update_administrative_units`

```{r}
#' Update RDB `administrative_units` table
#'
#' Updates the RDB's `administrative_units` table with data combined from the `subnational_entities`, `countries`, `subnational_entities` and `municipalities`
#' tables.
#'
...
-   It doesn't make much sense to return the return value of `pg_tbl_update()` since evaluating it will throw an `Invalid connection` error if
    `disconnect = TRUE` (the default). Hence, we just return `data`.
...
-   implement PGSQL data backup -\> restoral

    -   backup:

        ``` r
        connection <- connect()
        file_name_bkp <- "tmp_rdb_dm_bkp.rds"
        ...
        dm_bkp <-
          ...
          dm::dm_select_tbl(!!tbl_names_to_bkp) |>
          # TODO: exclude non-PK `GENERATED ALWAYS` cols
          dplyr::collect() |>
          # we write `dm_bkp` to disk and read it back in before restoral in order to be prepared for any failures in between
          saveRDS(file = file_name_bkp)
        ```

    -   restore:

        NOTE: we need to populate the tbls in the right order to not violate FK constraints. we can derive this order from `dm::dm_get_all_fks()`.

        ``` r
        dm_bkp <- readRDS(file = file_name_bkp)
        ...
        tbl_names_todo <- tbl_names_to_bkp
        ...
        while (length(tbl_names_todo) > 0L) {
          ...
          # restore next tbls
          tbl_names_next |>
            purrr::walk(\(tbl_name) rdb::update_tbl(data = dm_bkp[[tbl_name]],
                                                    tbl_name = tbl_name,
                                                    connection = connection,
                                                    disconnect = FALSE))
...
-   Sometimes, `reset_rdb()` fails with

    ```         
    Error in map(.x, .f, ..., .progress = .progress) :                           
      ℹ In index: 13.
    Caused by error in `h()`:
    ! error in evaluating the argument 'res' in selecting a method for function 'dbClearResult': Failed to fetch row : ERROR:  canceling statement due to user request
    ✖ Recreating PostgreSQL database… failed [6.6s]
    ```

    The error suggests that it occurred during `pg_init_db()`. We've tried adding a sleep delay of 2s (to wait long enough for `pg_reset_db()` to fully
    propagate) without success. 😕

    Note that `reset_rdb()` always finishes successfully on the second attempt.

    Update: This didn't occur anymore since at least 2024-08-18...

NOTES

-   When resetting only NocoDB via `rdb::reset_rdb(reset_db = FALSE, reset_tbls = FALSE)`, the `created/updated_by/at` columns aren't recognized as *system*
    columns anymore in the re-created NocoDB base, which breaks auto-fill of `created/updated_by/at` columns as well as M2M table detection (including the
    `created/updated_by/at`, the 5-column limit for M2M tables is exceeded).
...
#' @param quiet `r pkgsnip::param_lbl("quiet")`
#'
#' @return `hostname_nocodb`, invisibly.
#' @family admin
#' @family nocodb
#' @export
#'
...
reset_rdb <- function(hostname_nocodb = pal::pkg_config_val(key = "nocodb_host",
    ...
    cli::cli_alert_warning(paste0("{.strong CAUTION!}\nContinuing completely resets ",
                                  ifelse(reset_tbls,
                                         paste0("the RDB PostgreSQL ",
                                                ...
                                                "tables and "),
                                         ""),
                                  "the RDB NocoDB server {.field {hostname_nocodb}}. This results in loss of all ",
                                  ifelse(reset_tbls,
  ...
  if (reset_tbls) {
    ...
    create_nocodb_tbl_cols(hostname = hostname_nocodb)
...
-   This fn is mainly useful after running `pg_reset_db()` to render the corresponding read-only Neon-tech compute endpoint accessible again (TODO: find out
    whether it recovers automatically after a certain timeout period!).
...
Function to administrate the [RDB NocoDB server](https://admin.rdb.vote/).
...
#### !`create_nocodb_tbl_cols`

NOTES

-   Trying to set `nocodb::create_tbl_col(dt = "timestamp with time zone")` fails with HTTP `400 Bad Request`, thus we have to alter the column type afterwards
    via `pg_post_nocodb()`.
...
-   `nocodb::update_tbl_col(id_col = nocodb::tbl_col_id(id_tbl = nocodb::tbl_id(name = "referendums"), name = "attachments"), uidt = "Attachment")` etc.

    Ensure that no constraints get lost!
...
-   Improve status msg output for `quiet = FALSE` and adapt `reset_rdb()` accordingly.

NOTES:

-   Sensitive user account information is retrieved from a CSV file defined via pkg conf val `nocodb_user_account_csv_file`.
...
-   Use Fly.io RESTful API instead of CLI.

NOTES:

-   We assume only a single NocoDB instance (Fly machine).
...
Function to administrate the [RDB PostgREST server](https://api.rdb.vote/).
...
-   Unfortunately, we cannot sensibly determine if a subnational territory on sudd.ch is a subdivision like a province, i.e. a `subnational_entity_name_de`,
    *or* a `municipality_name_de`. There seems to be no semantic data included in any form which we could derive this from.
