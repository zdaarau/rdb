...
-   Sometimes, `reset_rdb()` fails with
    ...
    The error suggests that it occurred during `pg_init_db()`. Maybe this is a race condition and we have to wait long enough for `pg_reset_db()` to fully
    propagate, thus we've added 1s sleep... 🤞
