...
-   We can't rely on `pg_tbl_update()` here, since it assumes to be able to write to the PK column – which in case of tbl `municipalities` is
    [`GENERATED ALWAYS`](https://www.postgresql.org/docs/current/sql-createtable.html#SQL-CREATETABLE-PARMS-GENERATED-STORED) and thus read-only – i.e. it'll
    throw an error:

    ```         
    ! Failed to fetch row : ERROR:  cannot insert a non-DEFAULT value into column "id"
    DETAIL:  Column "id" is a generated column.
    ```

    Hence, we use custom code.
